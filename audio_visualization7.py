...
class AnimatedAudioVisualizer:
    def __init__(self, visualization_type='waveform', fps=30, background_image=None):
        self.visualization_type = visualization_type
        self.fps = fps
        self.background_image = background_image  # Path to the background image
    ...
    def make_frame(self, t, y, sr, total_duration):
        ...
        # ax.set_facecolor('black')  # Black background for the waveform axis

        if self.background_image:
            img = mpimg.imread(self.background_image)  # Read the image file
            ax.imshow(img, aspect='auto', extent=[0, len(y), -1, 1])  # Display the image
        else:
            # Fallback to a plain color if no background image is provided
        ...
        ax.plot(np.arange(start, end), y[start:end], color='blue')
...
if __name__ == "__main__":
    ...
    visualizer = AnimatedAudioVisualizer(visualization_type=args.type, fps=args.fps, background_image='E:/USB_vue_US/converter/bg.PNG')
