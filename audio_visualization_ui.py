...
class AnimatedAudioVisualizer:
    ...
    def make_frame(self, t, y, sr, total_duration):
        ...
        # x_smooth = np.convolve(y_displaced, np.ones(window_size)/window_size, mode='valid')
        ...
        # ax.plot(np.arange(start, end), y[start:end], color='white', alpha=0.5, lw=3)  # Original waveform
        # ax.plot(np.arange(start, end), y[start:end] + 0.01, color='white', alpha=0.1, lw=2)  # Smoothed waveform (lighter)
        ...
        # ax.plot(x_smooth, y_smooth, color='cyan', alpha=0.7, lw=2)
        # ax.plot(np.arange(start, end), y[start:end], color='white')
